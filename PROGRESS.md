...
- Added an isolated short-video playback concept demo screen, with ordinary playback UI separated from a hidden local-material configuration mode.
...
- Moved hidden demo video uploads out of localStorage and into IndexedDB Blob storage, with metadata-only `videoBlobKey` config, upload validation, and legacy `data:video` cleanup.
- Changed the hidden config shortcut from the browser-reserved source shortcut to `Ctrl+Shift+U`, while preserving `?page=admin&dev=1` and `Esc` return behavior.
...
- `npm.cmd run build` completed successfully after adding the short-video playback demo screen; browser verification confirmed ordinary mode hides upload/config controls and the hidden 3-video configuration panel can be opened from the lab shortcut.
...
- `npm.cmd run build` completed successfully after moving hidden uploaded videos from localStorage to IndexedDB Blob storage.
- Browser verification confirmed `?page=admin&dev=1` renders without blanking, shows `返回演示页` and `返回开发导航`, the dev header advertises `Ctrl + Shift + U`, that shortcut opens the admin page, `Esc` returns to `?page=demo&dev=1`, and the clean demo page still falls back to its placeholder when no video Blob is available.
